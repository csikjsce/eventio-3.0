...
export default function TicketScreen() {
  ...
  const [participantId, setParticipantId] = useState<string>("DEMO-001");
  ...
  useEffect(() => {
    ...
    setIsDarkMode(mq.matches || true); // default dark
        ...
        <div className="px-6 pt-2 pb-6">
          ...
          <div className="mt-4 bg-surface rounded-2xl p-4 border border-border">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-foreground font-bold text-xl font-poppins">
                  {timeLabel}
                </p>
              ...
              <div className="text-right">
                <p className="text-foreground font-bold text-xl font-poppins">
                  {eventData?.dates.length && eventData.dates.length > 1
                    ? new Date(eventData.dates[eventData.dates.length - 1]).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })
                    : timeLabel}
                </p>
                <p className="text-mute text-xs font-poppins mt-0.5">{dateLabel}</p>
              </div>
