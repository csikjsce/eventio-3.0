    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-background p-2 rounded-md shadow-md">
                ...
                <p className="font-fira text-foreground text-sm">{`${payload[0].value}`}</p>
            </div>
        );
    }
    return null;
...
function Statistics() {
    ...
    const keyInsights = useMemo(() => {
        ...
        const mostActiveBranch = Object.entries(branchTotals).reduce(
            (max, [branch, count]) =>
                count > max.count ? { branch, count } : max,
            { branch: "N/A", count: 0 },
        );
        ...
        return {
            ...
            averageAttendance,
            mostActiveBranch:
                branchAbbreviations[mostActiveBranch.branch] || "N/A",
            genderRatio,
        };
    }, [statsData]);
    ...
    async function downloadAttendance(eventId: string, eventName: string) {
        try {
            const response = await axios.request({
                baseURL: import.meta.env.VITE_APP_SERVER_ADDRESS,
                url: `/api/v1/event/p/attendance-report/${eventId}`, // PDF endpoint
                method: "GET",
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none w-full bg-card text-foreground px-4 py-2 pr-8 rounded-md focus:outline-none focus:ring-2 focus:ring-primary"
                            >
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        if (eventsToDownload.length === 0) {
                                            alert(
                                                "No events available to download attendance.",
                                            );
                                            return;
                                        }
                                        ...
                                        if (eventsToDownload.length > 1) {
                                            if (
                                                !confirm(
                                                    `Download attendance for ${eventsToDownload.length} events?`,
                                                )
                                            ) {
                                                return;
                                            }
...
function InsightCard({
    ...
    return (
        <div className="bg-card p-4 rounded-md shadow-md">
            ...
            <p className="font-fira text-foreground text-2xl font-bold">
                {value}
            </p>
            {subvalue && (
                <p className="font-fira text-foreground text-sm">{subvalue}</p>
            )}
        </div>
    );
}
                    ...
                    >
                        {data.map((entry, index) => (
                            <Cell
                                key={`cell-${index}`}
                                fill={
                                    COLORS.branch[index % COLORS.branch.length]
                                }
                            />
                        ))}
                    </Pie>
        ...
        if (active && payload && payload.length) {
            return (
                <div className="bg-background p-2 rounded-md shadow-md">
                    ...
                    <p className="font-fira text-foreground text-sm">
                ...
                >
                    {yearData.map((entry, index) => (
                        <Cell
                            key={`cell-${index}`}
                            fill={
                                COLORS.barChart[index % COLORS.barChart.length]
                            }
                        />
                    ))}
                </Bar>
            ...
            return (
                <div className="bg-background p-2 rounded-md shadow-md">
                    ...
                    {payload.map((entry, index) => (
                        <p
                            key={index}
                            className="font-fira text-foreground text-sm"
                        >
                ...
                <BarChart data={data} layout="vertical">
                    ...
                    {events.map((event, index) => (
                        <Bar
                            ...
                            fill={
                                COLORS_EXTENDED[index % COLORS_EXTENDED.length]
                            }
                        />
                    ))}
                </BarChart>
